...
//
// Brandon Haak
// Adapted from: Mohitkumar P. Ghia (Project 1 solution)
        ...
        using (var file = new System.IO.StreamReader("courses.csv"))
         {
               while (!file.EndOfStream)
               {
                    ...
                    string coursetype = values[5];
                    string day = values[6];
                    string time = values[7];
                    int size = Convert.ToInt32(values[8]);
                    string sqlq = string.Format(@"
INSERT INTO Courses(CRN, Department, CourseNum, Semester, Year, CourseType, Day, ClassTime, ClassSize, NumEnrolled)
Values({0}, '{1}', {2}, '{3}', {4}, '{5}', '{6}', '{7}', {8}, 0);
",
crn, department, coursenum, semester, year, coursetype, day, time, size);
